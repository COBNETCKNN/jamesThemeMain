...
function portfolioTheme_files() {
    //enqueue CSS
    wp_enqueue_style('mainCSS', get_template_directory_uri() . '/css/main.css', array(), '1.7');
    wp_enqueue_style('fontAwesomeCSS', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css');
    if(is_single()){
      wp_enqueue_style('modalCSS', get_template_directory_uri() . '/css/modal.css', array(), '1.7');
    }

    //enqueue JS
    wp_enqueue_script('jquery');
    wp_enqueue_script('mainJS', get_stylesheet_directory_uri() . '/js/main.js', array(), 1.7, true);
    wp_enqueue_script('fontAwesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/js/all.min.js');
    wp_enqueue_script('masonryLayout', get_stylesheet_directory_uri() . '/js/masonry.pkgd.min.js', array(), 1.7, true);
...
function add_ajax_scripts() {
  wp_enqueue_script( 'ajax_term', get_stylesheet_directory_uri() . '/ajax/filter-ajax.js', array('jquery'), 1.7, true );
  wp_enqueue_script('ajax_modal', get_stylesheet_directory_uri() . '/ajax/modal-ajax.js', array('jquery'), 1.7, true);
  wp_enqueue_script('ajax_term_ce', get_stylesheet_directory_uri() . '/ajax/ce-ajax-filter.js', array('jquery'), 1.7, true);
